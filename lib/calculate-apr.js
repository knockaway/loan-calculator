...
export function calculateApr({
  ...
  if (compoundingFrequency !== paymentFrequency) {
    interestRate = convertCompoundingFrequency({
      interestRate,
  ...
  const loanReceived = loanAmount - upfrontFees;

  // Intuitively, we're trying to find the interest rate that works out to the same monthly payment, and hench
  ...
  function f(x) {
    return calculatePayment({ presentValue: loanReceived, interestRate: x, numberOfPayments }) - periodPayment;
  }
  ...
  function fp(x) {
    ...
    return (
      (loanReceived * onePlusXToNm1 * (x * onePlusXToN + onePlusXToN - numberOfPayments * x - x - 1)) /
      Math.pow(onePlusXToN - 1, 2)
    );
  }

  const payPeriodAPR = NewtonRaphsonMethod(f, fp, periodInterestRate);
  let apr = payPeriodAPR * paymentFrequency;

  // convert back to the compounding frequency
  if (compoundingFrequency !== paymentFrequency) {
    apr = convertCompoundingFrequency({
      interestRate: apr,
